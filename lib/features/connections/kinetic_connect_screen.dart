...
class _KineticConnectScreenState extends State<KineticConnectScreen> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.black, // Pure black so the neon red pops flawlessly
        ...
        child: Stack(
          ...
          if (_currentState != ConnectState.listening)
            Positioned.fill(
              child: TweenAnimationBuilder<double>(
                ...
                curve: Curves.fastOutSlowIn, // Extreme smoothness, fluid like NameDrop liquid
          ...
          if (_currentState != ConnectState.listening)
            Positioned.fill(
              child: TweenAnimationBuilder<double>(
                ...
                tween: Tween(begin: 1.0, end: _currentState == ConnectState.success ? 0.0 : 1.0),
                duration: const Duration(milliseconds: 2000),
                curve: Curves.easeOutCubic, // Butter-smooth tightening, zero harsh recoil
          ...
          if (_currentState == ConnectState.success)
             TweenAnimationBuilder<double>(
               ...
               curve: Curves.fastOutSlowIn, // Smoothly swells and beautifully drifts away
               builder: (context, value, child) {
                ...
                builder: (context, scale, child) {
                  return Transform.scale(
                    ...
                    child: ElevatedButton.icon(
                      onPressed: () => Get.off(() => const ConnectionsScreen()),
                      icon: const Icon(Icons.check_circle_rounded, color: Colors.white),
                      label: const Text('View Connection', style: TextStyle(color: Colors.white, fontWeight: FontWeight.w600, fontSize: 16, letterSpacing: -0.5)),
  ...
  Widget _buildVisualizer() {
    ...
    return AnimatedBuilder(
      ...
      child: AnimatedContainer(
        ...
        child: isSuccess ? TweenAnimationBuilder<double>(
          tween: Tween(begin: 0.0, end: 1.0),
          duration: const Duration(milliseconds: 3200), // Extended slightly for full alchemy phases
          curve: Curves.linear, // Drive perfectly linearly; we use explicit math for phases
          builder: (context, linearValue, child) {
            ...
            final double transitionPhase = ((linearValue - 0.4) / 0.3).clamp(0.0, 1.0);
  ...
  Widget _buildStatusText() {
    ...
    return Column(
      children: [
        AnimatedSwitcher(
          duration: const Duration(milliseconds: 1800), // Slow cinematic crossfade
            ...
            ? RichText(
                ...
                text: TextSpan(
                  children: [
                    const TextSpan(
                      text: 'Connected with\n',
                      style: TextStyle(color: Colors.white70, fontSize: 16, fontWeight: FontWeight.w400, height: 1.5, letterSpacing: 0.5),
                    ),
                    TextSpan(
                      text: _acceptedPeer!.offlineUsername,
                      style: const TextStyle(color: Colors.white, fontSize: 32, fontWeight: FontWeight.w800, letterSpacing: -1.0),
                    ),
                  ],
                ),
              )
...
class _AlchemicalDustPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < dustCount; i++) {
      ...
      final double easedSuck = pow(suckInProgress, 0.5 + random.nextDouble()).toDouble();
      
      // Animate from off-screen into the solid disk shape
      double currentRadius = initialRadius + (finalTargetRadius - initialRadius) * easedSuck;
      ...
      final double swirlingAngle = angle + (progress * pi * 4 * random.nextDouble() * spinSpeed);
      ...
      canvas.drawCircle(Offset(px, py), (baseSize * sizeMultiplier).clamp(0.0, 12.0) + 4, glowPaint);
      canvas.drawCircle(Offset(px, py), (baseSize * sizeMultiplier).clamp(0.0, 12.0), corePaint);
    }
  }

  @override
  bool shouldRepaint(covariant _AlchemicalDustPainter oldDelegate) => oldDelegate.progress != progress;
}
...
class _EmberPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < 25; i++) {
      ...
      double currentY = (startY - (time * size.height * speedMult)) % size.height;
      if (currentY < 0) currentY += size.height; 
      
      // Gentle horizontal sway
      double currentX = startX + sin((time * 2 * pi * speedMult) + i) * 15.0; 
      
      canvas.drawCircle(Offset(currentX, currentY), random.nextDouble() * 4 + 1, paint);
    }
  }

  @override
  bool shouldRepaint(covariant _EmberPainter oldDelegate) => oldDelegate.time != time;
}
...
class _RedStringPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final corePaint = Paint()
      ..color = const Color(0xFFFF6675) // Noticeably lighter inner core
      ..style = PaintingStyle.stroke
      ..strokeWidth = 1.5
      ..strokeCap = StrokeCap.round;
    ...
    for (int i = 1; i <= segments; i++) {
      ...
      final double xOffset = sin((i * 0.15) - wavePhase) * currentAmplitude * taper;
